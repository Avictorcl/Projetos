/* Para compilar em terminal, digitar gcc lab.c -o lab -pthread */
...
// Matrizes globais para garantir a complexidade O(1)
struct tm matriz_horarios[100][100];
int matriz_visitados[100][100] = {0}; // 0 = não visitado, 1 = visitado
...
void* ler_valores_de_sensores(void* arg) {
    ...
    while (thread_status == 1) {
       ...
       /**** Implementar a leitura dos sensores aqui ****/
        ...
        visita = ultima_visita();
        int x_lido = visita.coordenada.x;
        int y_lido = visita.coordenada.y;
        
        // Salva a hora exata da visita na nossa matriz global O(1)
        matriz_horarios[x_lido][y_lido] = *(visita.hora);
        matriz_visitados[x_lido][y_lido] = 1; // Marca a coordenada como visitada
...
void horario_visitado(Visita horario){
    ...
    if(x >= 0 && x < 100 && y >= 0 && y < 100){
        ...
        if (matriz_visitados[x][y] == 1){
            ...
            printf("O ponto (%d, %d) não foi registrado\n", x, y); 
        } 

    } 
    else{
        printf("As coordenadas escritas não estão dentro do limite de (0 a 99)\n"); 
    }
...
void menor_ponto_visitado_apos(Visita apos){
    int x, y;
